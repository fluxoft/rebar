...
abstract class UserModel extends Model {
	...
	protected function setPassword ($password) {
		...
		if ($hash != $this->properties[$this->authPasswordColumn]) {
			$query = 'UPDATE '.$this->dbTable.' '.
				'SET '.$this->authPasswordColumn.' = :password '.
				'WHERE id = :userID';
			$params = array(
				':password' => $hash,
				':userID' => $this->properties[$this->idProperty]
			);
			$this->writer->Update($query, $params);
			$this->properties[$this->authPasswordColumn] = $hash;
		}
	}
